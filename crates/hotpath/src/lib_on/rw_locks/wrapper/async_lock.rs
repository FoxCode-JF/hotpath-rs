//! Instrumented wrapper for [`async_lock::RwLock`].
...
impl<T> RwLock<T> {
    ...
    pub fn new(value: T) -> Self {
        ...
        Self::__new_instrumented(AlRwLock::new(value), source, None)
    }
    ...
    fn read_guard<'a>(
        &'a self,
        inner: async_lock::RwLockReadGuard<'a, T>,
        wait_nanos: u64,
    ) -> RwLockReadGuard<'a, T> {
        RwLockReadGuard {
            inner,
    ...
    fn write_guard<'a>(
        &'a self,
        inner: async_lock::RwLockWriteGuard<'a, T>,
        wait_nanos: u64,
...
impl<T> InstrumentRwLock for AlRwLock<T> {
